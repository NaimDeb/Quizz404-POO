...
class QCMManager
{
    ...
    public static function generateDisplayAllQuizzes()
    {
        ...
        <h1 class="text-5xl max-sm:text-4xl font-first-font text-center">CHOISIS TON THÈME !</h1>

<div class="pt-6 flex justify-center">
    <div class="flex gap-8 flex-wrap max-lg:flex-nowrap max-lg:overflow-x-auto max-lg:scrollbar-hide">
        <?php
...
<a class="size-[300px] hover:scale-110 transition-all border-black border-[2px] shrink-0 flex justify-center items-center" href="quizz.php?id=<?= htmlspecialchars($QCM->getId()) ?>">
    ...
    ?>
        ...
        <div class="pt-[70px] container mx-auto p-8 bg-gray-100 rounded-lg shadow-lg overflow-y-auto text-center max-h-screen">
    <h2 class="text-3xl font-bold text-center text-gray-800 mb-6">
      Quizz<br><?= htmlspecialchars($qcm->getNom()) ?>
    </h2>
        ...
        <?php foreach ($qcm->getQuestion() as $index => $question): ?>
            <div class="question-card bg-white p-6 mb-6 rounded-lg shadow-sm" data-question-index="<?= $index ?>" style="display: <?= $index === 0 ? 'block' : 'none' ?>;">
                <h3 class="text-2xl font-semibold text-gray-700 mb-4">
                  Question : <span><?= $index + 1 ?></span> / <span><?= $numberOfQuestions ?></span><br>
                  <?= htmlspecialchars($question->getIntitule()) ?>
                </h3>
                ...
                <ul class="flex flex-wrap justify-center gap-4">
                    ...
                    <?php foreach ($question->getAnswers() as $answer): ?>
                        <li class="max-sm:w-1/2 answer-item text-gray-600 text-xl bg-gray-200 text-center px-4 py-2 border-2 border-gray-300 rounded-lg cursor-pointer max-w-full basis-full lg:basis-[45%] transition-all transform hover:scale-110" data-is-right="<?= $answer->getisRightAnswer() ? 'true' : 'false' ?>" data-answer="<?= htmlspecialchars($answer->getIntitule()) ?>">
                            <?= htmlspecialchars($answer->getIntitule()) ?>
                        </li>
                    <?php endforeach; ?>
                </ul>
                <p class="correct-answer text-lg text-gray-600 mt-4 hidden">
                La bonne réponse est : <span class="correct-answer-text"></span>
                </p>
                <p class="explanation text-lg text-gray-600 mt-4 hidden">
                <?= htmlspecialchars($question->getExplanation()) ?>
                </p>
            </div>
        <?php endforeach; ?>
    </div>
